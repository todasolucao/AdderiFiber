<?php
        ...
        if($hash){
            if($this->pagamento_model->transacao($hash)->r_situacaoTransacao == 'null'){         
                //$this->pagamento_model->inicia_transacao($hash, 'I', $external, date('Y-m-d'), 'Crédito', '', date('Y-m-d H:i:s'), $pagamento->r_valorTotal, $pagamento->r_valorTotal, $pagamento->r_valorTotal);
            }elseif($this->pagamento_model->transacao($hash)->r_situacaoTransacao == 'B'){
                redirect('bloqueado');
                exit();
            }elseif($this->pagamento_model->transacao($hash)->r_situacaoTransacao == 'L'){
                redirect('liquidada');
                exit();
            }elseif($this->pagamento_model->transacao($hash)->r_situacaoTransacao == 'I'){
                redirect('iniciada');
                exit();
            }
        }

	    $valorx = str_replace(",",".", $pagamento->r_valorTotal);
    ...
    <div class="col">
      <div class="pagamento-container">
        <div class="payment-title">
          <h3>Dados do Cartão</h#>
        </div>
        <div class="container-wrapper preload">
               ...
               <div class="creditcard">
                        ...
                        <g>
                        <text transform="matrix(1 0 0 1 574.4219 433.8095)" id="svgexpire" class="st2 st5 st9">01/2025</text>
...
<form id="form1" action="autoriza_cartao_credito" class="form-container" method="POST">
		
            
            <input type="hidden" name="ddd" class="form-control  col-sm-2" value="11">
            <input type="hidden" name="telefone" class="form-control  col-sm-10" value="00000000">
				
                   
                        <div class="field-container">
                             <label for="name">Nome</label>
							 <input type="text" id="name"  maxlength="20" name="titular_nome" data-checkout="cardholderName" value="">
                        </div>
                        
                         <div class="field-container">
                             <label for="cpf">CPF</label>
                            <input id="cpf" name="titular_doc" value=""  data-checkout="docNumber" maxlength="20" type="text">
                        </div>
                        
						<div class="field-container">
                                <label for="cardnumber">Número do Cartão</label>
							    <input type="text" name="cartao_numero" id="cardnumber" class="cartao" data-checkout="cardNumber" autocomplete=off value="">
						...
						<div class="field-container">
                                    <label for="expirationdate">Mês</label>
                                    <select name="cartao_mes" id="cartao_mes" data-checkout="cardExpirationMonth">
                                        <option value="">Selecione...</option>
                                        <?php for ($i = 1; $i <= 12; $i++) {
                                            echo "<option value='" . $i . "' " . (($i == 01) ? "selected" : "") . ">{$i}</option>";
                                        }
                                        ?>
                                    </select>  
						</div>
						<div class="field-container">
                                    <label for="cartao_ano">Ano</label>
                                    <select name="cartao_ano" id="cartao_ano" data-checkout="cardExpirationYear">
                                        <option value="">Selecione...</option>
                                            <?php for ($i = date("Y"); $i <= date("Y") + 15; $i++) {
                                                echo "<option value='" . $i . "' " . (($i == 2025) ? "selected" : "") . ">{$i}</option>";
                                            } ?>
                                    </select> 
						</div>
							<div class="field-container">
                                <label for="securitycode">CVV</label>
                                <input id="securitycode" type="text" name="cartao_codigo" id="cartao_codigo" data-checkout="securityCode" value="" maxlength="3">
                            </div>
					        <div class="field-container">
                                    <label for="parcelamento">Parcelas</label>
                                    <select id="parcelamento" name='parcelas_quantidade'></select> 
							 </div>
						
					<div class="field-container" style="margin-top: 16px">
                        <button id="pagar-btn" class="btn btn-color" type="submit">Pagar</button>
                        <a href="https://pagamentos.fiberoficial.com.br/pagamento/token/<?php echo $hash; ?>" class="btn">Cancelar</a>
                    </div>
						
                    <input type="hidden" name="total" class="form-control total" value="<?php echo $valorx;?>">
    	            <input type="hidden" name="titular_doc_tipo" data-checkout="docType" value="<?php echo $tipo; ?>"/>
                    <input type="hidden" data-checkout="siteId" value="FIBER"/>
                    <input type="hidden" name="paymentMethod" value="creditcard">
                    <input type="hidden" name="nome" value="Pagamento via Link">
                    <input type="hidden" name="total" class="total" value="<?php echo $valorx;?>">
...
Mercadopago.setPublishableKey("<?php echo $pagamento->r_publicKey; //TEST-a04a055c-8d6a-4966-b653-e2adf4073409 ?>"); 
...
window.onload = function () {
  ...
  const cartao_ano = document.getElementById('cartao_ano');
  const cartao_mes = document.getElementById('cartao_mes');
  ...
  var cardnumber_mask = new IMask(cardnumber, {
    ...
    dispatch: function (appended, dynamicMasked) {
      ...
      for (var i = 0; i < dynamicMasked.compiledMasks.length; i++) {if ("") break;
  ...
  var expirationMes_mask = new IMask(cartao_mes, {
     mask: '00' });
  var expirationAno_mask = new IMask(cartao_ano, {
     mask: '0000' });
  ...
  expirationMes_mask.on('accept', function () {
    if (expirationMes_mask.value.length == 0) {
      document.getElementById('svgexpire').innerHTML = '01/2025';
    } else {
      document.getElementById('svgexpire').innerHTML = expirationMes_mask.value + "/"+ expirationAno_mask.value;
		
    }
  });
  
  expirationAno_mask.on('accept', function () {
    if (expirationAno_mask.value.length == 0) {
      document.getElementById('svgexpire').innerHTML = '01/2025';
    } else {
      document.getElementById('svgexpire').innerHTML = expirationMes_mask.value + "/"+ expirationAno_mask.value;
  ...
  cartao_ano.addEventListener('focus', function () {
    document.querySelector('.creditcard').classList.remove('flipped');
  });

  cartao_mes.addEventListener('focus', function () {
    document.querySelector('.creditcard').classList.remove('flipped');
  });
